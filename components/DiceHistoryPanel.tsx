        ...
        <div className="history-list">
          {rolls.length === 0 ? <div className="history-empty"><Dices /><strong>Nenhuma rolagem ainda</strong><span>Os resultados desta campanha aparecerão aqui.</span></div> : rolls.map((roll) => (
            <article key={roll.id}>
              ...
              <div><strong>{nameOf(roll)}</strong><span>{roll.quantity ?? 1}d{roll.sides}{roll.modifier ? ` ${roll.modifier > 0 ? "+" : ""}${roll.modifier}` : ""} · {validationLabels[roll.validationMode ?? "sum"]}</span>{roll.results && roll.results.length > 1 ? <small>[{roll.results.join(", ")}]</small> : null}</div>
