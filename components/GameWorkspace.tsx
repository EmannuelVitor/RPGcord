...
export function GameWorkspace({ user, campaign, onCampaigns, onLeaveCampaign, onDeleteCampaign, onRemoveMember, appearance, onSignOut, onTutorial }: {
  ...
  function renderPanel(panel: WorkspaceTabKey) {
    if (isWhisperTab(panel)) {
      ...
      return <SessionChat embedded campaignId={campaign.id} campaignName={campaign.name} user={user} messages={privateMessages} participants={game.participants} tokens={game.tokens} privateWith={partner} isGM={game.isGM} onSend={game.sendChatMessage} onEdit={game.editChatMessage} onDelete={game.deleteChatMessage} onTyping={game.setTyping} onClear={game.clearChat} onClose={() => closePanel(panel)} />;
    }
