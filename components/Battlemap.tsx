...
import { Eraser, Eye, Lightbulb, Lock, Minus, Plus, RotateCcw, ScrollText, SlidersHorizontal, Sun, Trash2, Users, X } from "lucide-react";
...
type Props = {
  scene: Scene; tokens: MapToken[]; userId: string; isGM: boolean; hasCharacter?: boolean;
...
export function Battlemap(props: Props) {
  const { scene, tokens, userId, isGM, hasCharacter = true, onMove, onToggleTokenLock = () => undefined, onRequestCharacter = () => undefined, onRevealArea, onClearRevealed, onMoveLight, onCreateLight, onUpdateLight, onDeleteLight, onSetGlobalVision, onSetTokenVision } = props;
  ...
  function tokenStart(event: PointerEvent<HTMLButtonElement>, token: MapToken) {
    ...
    event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); setDrag({ id: token.id, ...constrainTokenPoint(point(event)) });
  }
    ...
    <div className="map-viewport" ref={viewportRef}>
      <div className={"map-scroll " + (panning ? "is-panning" : "")} ref={scrollRef} onWheel={wheel} onPointerDown={panStart} onPointerMove={panMove} onPointerUp={panEnd} onPointerCancel={panEnd} onContextMenu={(event) => event.preventDefault()}>
        <div className="map-stage" style={stage}><div ref={mapRef} className={"battle-map " + (revealMode ? "reveal-mode " : "") + (lightMode ? "light-placement-mode" : "")} onPointerUp={mapClick} style={mapSize}>
          ...
          {tokens.map((token) => { const p = drag?.id === token.id ? drag : token; const canControl = isGM || token.ownerId === userId; return <button className={"map-token " + token.kind + (token.locked ? " locked" : "") + (drag?.id === token.id ? " dragging" : "")} key={token.id} style={{ left: p.x + "%", top: p.y + "%", "--token-color": token.color } as React.CSSProperties} onPointerDown={(event) => tokenStart(event, token)} onPointerMove={(event) => drag && setDrag({ id: drag.id, ...constrainTokenPoint(point(event)) })} onPointerUp={tokenEnd} onDoubleClick={(event) => { if (canControl) { event.stopPropagation(); onToggleTokenLock(token.id); } }} onClick={(event) => { if (canControl && token.kind === "hero") { event.stopPropagation(); setSelectedToken(token.id); if (isGM) setVisionOpen(true); } }} title={canControl ? token.name + " · duplo clique para " + (token.locked ? "desbloquear" : "bloquear") : token.name}>{token.imageUrl ? <img src={token.imageUrl} alt="" draggable={false} /> : token.initials}{token.locked ? <Lock className="token-lock-icon" size={12} /> : null}<span>{token.name}</span></button>; })}
          {scene.fogEnabled && <svg className={"fog-of-war " + (isGM ? "gm-fog" : "")} viewBox="0 0 100 100" preserveAspectRatio="none"><defs><radialGradient id={visionId}><stop offset="0%" stopColor="#000" /><stop offset="58%" stopColor="#000" /><stop offset="82%" stopColor="#777" /><stop offset="100%" stopColor="#fff" /></radialGradient>{lights.filter((light) => light.enabled).map((light) => { const shade = Math.round(255 * (1 - light.intensity)); return <radialGradient id={"mask-" + light.id} key={light.id}><stop offset="0%" stopColor={"rgb(" + shade + "," + shade + "," + shade + ")"} /><stop offset={bright(light) / dim(light) * 100 + "%"} stopColor={"rgb(" + shade + "," + shade + "," + shade + ")"} /><stop offset="100%" stopColor="#fff" /></radialGradient>; })}<mask id={fogId}><rect width="100" height="100" fill="#fff" />{visibleHeroTokens.map((token) => { const p = drag?.id === token.id ? drag : token; return <circle key={token.id} cx={p.x} cy={p.y} r={token.visionRadius ?? vision} fill={"url(#" + visionId + ")"} />; })}{(scene.revealedAreas ?? []).map((area) => <circle key={area.id} cx={area.x} cy={area.y} r={area.radius} fill={"url(#" + visionId + ")"} />)}{lights.filter((light) => light.enabled).map((light) => { const p = lightDrag?.id === light.id ? lightDrag : light; return <circle key={light.id} cx={p.x} cy={p.y} r={dim(light)} fill={"url(#mask-" + light.id + ")"} />; })}</mask></defs><rect className="fog-overlay" width="100" height="100" mask={"url(#" + fogId + ")"} style={{ opacity: Math.max(.15, 1 - (scene.ambientLight ?? 0) * .8) }} /></svg>}
